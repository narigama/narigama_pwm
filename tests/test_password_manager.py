...
@given("I create a password manager", target_fixture="password_manager")
def password_manager():
    yield narigama_pwm.PasswordManager()


@given("I create a password manager with defaults", target_fixture="password_manager")
def password_manager_default():
    yield narigama_pwm.PasswordManager.default()


@given(parsers.parse("I install the {algo} module"))
def password_manager_load_algorithm(
    password_function_factory,
    password_manager: narigama_pwm.PasswordManager,
    algo: str,
):
    password_manager.install(password_function_factory(algo))
    yield


@when(parsers.parse("I reinstall the {algo} module it should fail"))
def password_manager_reinstall_algo_crashes(
    password_function_factory,
    password_manager: narigama_pwm.PasswordManager,
    algo: str,
):
...
scenarios("features/password_manager.feature")
